...
export default function ArchiveModels({
  onReady,
}: ArchiveModelsProps) {
  const floor = useGLTF(
    "/models/archive/PillarsAndFloor.glb"
  );

  const rocks = useGLTF(
    "/models/archive/Rocks.glb"
  );

  const pedestal = useGLTF(
    "/models/archive/PedestalAndCenterMid.glb"
  );

  const crystal = useGLTF(
    "/models/archive/Crystal.glb"
  );

  const activated = useCrystalStore(
  ...
  const setProgress = useCrystalStore(
    (state) => state.setActivationProgress
  );
  ...
  useEffect(() => {
    ...
    const bounds =
      new THREE.Box3().setFromObject(root.current);

    const crystalPosition =
      new THREE.Vector3();

    crystalGroup.current.getWorldPosition(
      crystalPosition
    );
  ...
  useFrame((state, delta) => {
    ...
    if (activated) {
      ...
      floatAmount = THREE.MathUtils.lerp(
        2,
        6,
        next
      );
    ...
    crystalGroup.current.rotation.y +=
      rotationSpeed;

    crystalGroup.current.position.y =
      Math.sin(t * 1.8) * floatAmount;
    ...
    if (crystalLight.current) {
      crystalLight.current.intensity =
        lightIntensity +
        Math.sin(t * 5) * 2;
    }
  });
